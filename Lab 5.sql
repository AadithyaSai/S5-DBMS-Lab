...
SELECT book.title, fine AS fine
FROM book, (
	SELECT title, SUM(late_fee) AS fine
	FROM book_return, book_issue
	WHERE book_issue.issue_id=book_return.issue_id
	GROUP BY book_id	
	) AS fine_list
WHERE book.book_id = fine_list.book_id;
